...
export default function ConversationScreen() {
  ...
  const downloadImage = useCallback(async (uri?: string) => {
    ...
    try {
      if (Platform.OS === 'web') {
        // Web: create an anchor and click it
        const a = document.createElement('a')
        a.href = uri
        // Try to infer filename
        const parts = uri.split('/')
        a.download = parts[parts.length - 1].split('?')[0] || 'image.jpg'
      ...
      // Native (Expo): download to cache, then save to media library
      const { status } = await MediaLibrary.requestPermissionsAsync()
      if (status !== 'granted') {
        Alert.alert('صلاحية مفقودة', 'يتطلب حفظ الصورة صلاحية الوصول إلى الوسائط.')
        return
      }
      ...
      if (downloaded.status !== 200 && downloaded.status !== 0) {
        // Some endpoints return 0 for success on Android? tolerate 0 as success in some environments
      }
      const asset = await MediaLibrary.createAssetAsync(downloaded.uri)
      await MediaLibrary.createAlbumAsync('Tribble', asset, false).catch(() => null)
      Alert.alert('تم التنزيل', 'تم حفظ الصورة في ألبوم الصور.')
    } catch (err) {
      console.error('downloadImage error', err)
      Alert.alert('خطأ', 'تعذر تنزيل الصورة.')
    }
  }, [])
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <Modal visible={!!previewImage} transparent animationType="fade" onRequestClose={() => setPreviewImage(null)}>
        <TouchableOpacity activeOpacity={1} onPress={() => setPreviewImage(null)} style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.95)', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <View style={{ position: 'absolute', top: 40, right: 20, backgroundColor: 'rgba(0,0,0,0.6)', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 16, flexDirection: 'row-reverse', gap: 8 }}>
            <TouchableOpacity onPress={() => setPreviewImage(null)} style={{ paddingHorizontal: 8 }}>
              <Text style={{ color: '#fff', fontFamily: 'bold' }}>إغلاق</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => downloadImage(previewImage ?? undefined)} style={{ paddingHorizontal: 8 }}>
              <Text style={{ color: '#fff', fontFamily: 'bold' }}>تنزيل</Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>
