...
import { getConversationsWithLastMessage, getUnreadCount, subscribeMessages } from '@/lib/chat'
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (user?.id) {
        ...
        setStories(s)
        // load unread messages count (sum across conversations)
        try {
          // get database user id from profile (getUser above)
          const meId = profile?.id ? String(profile.id) : null
          if (meId) {
            const convs = await getConversationsWithLastMessage(meId)
            const entries = await Promise.all(convs.map(async (c) => await getUnreadCount(c.conversation.id, meId)))
            const totalUnread = entries.reduce((a, b) => a + (typeof b === 'number' ? b : 0), 0)
            setUnreadMessages(totalUnread)
            // cleanup previous subs
            Object.values(subsRef.current).forEach(s => { try { s.unsubscribe() } catch {} })
            subsRef.current = {}
            // subscribe to each conversation to keep badge live
            convs.forEach(cs => {
              try {
                subsRef.current[cs.conversation.id] = subscribeMessages(cs.conversation.id, async (m) => {
                  // if incoming unread, refresh that conv unread count and total
                  try {
                    const cnt = await getUnreadCount(cs.conversation.id, meId)
                    // recalc total by fetching current counts for all conv ids (simple, safe)
                    const ids = convs.map(c => c.conversation.id)
                    const all = await Promise.all(ids.map(id => getUnreadCount(id, meId)))
                    const tot = all.reduce((a, b) => a + (typeof b === 'number' ? b : 0), 0)
                    setUnreadMessages(tot)
                  } catch (e) { console.log('subscribeMessages unread refresh error', e) }
                })
              } catch {}
            })
          } else {
        ...
        } catch (e) { console.log('load unread messages error', e); setUnreadMessages(0) }
     ...
     style={[styles.container, {padding: 0, flex: 0}]}>
      <SignedIn>
        <FlatList
          ...
          ListHeaderComponent={
            <>
              <Header unreadCount={unreadCount} unreadMessages={unreadMessages} />
              {loading ? (
                <SkeletonStoryRow />
              ) : (
              <FlatList
