import { dialogService } from '@/app/components/dialouge'
...
export default function EditCommunityScreen() {
  ...
  const onDelete = async () => {
    if (!user?.id) return Alert.alert('خطأ', 'يرجى تسجيل الدخول أولاً')
    if (!params.id) return Alert.alert('خطأ', 'معرّف المجتمع غير موجود')
    // use dialogService to show a confirmation in our custom dialog
    dialogService.showFromAlertArgs('حذف المجتمع', 'هل أنت متأكد من حذف المجتمع؟ لا يمكن التراجع.', [
