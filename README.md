...
To manually verify the image download feature added to the conversation view:

- Web (Expo for Web / Browser):
   1. Open the app in a browser (`expo start --web` or your usual dev flow).
   2. Open a conversation that contains an image.
   3. Tap the image to open the preview modal, then click the `تنزيل` (Download) button in the top-right. The browser should prompt a file download.

- Native (Expo on iOS/Android):
   1. Run the app on a device or simulator (`expo start` then scan the QR or run on emulator).
   2. Open a conversation with an image.
   3. Tap the image to open the preview modal, then tap `تنزيل` (Download). The app will ask for permission to access media; accept it. The image will be saved to the device's photo gallery (in an album named "Tribble" if supported).

If permission is denied on native, grant storage/media permissions from system settings and retry.
